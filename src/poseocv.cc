...
namespace poseocv
{
   void homography_pose(const cv::Mat& intrinsics, std::vector<std::pair<cv::Point3d, cv::Point3d>>&  pts,
                        std::vector<cv::Mat>& rotations, std::vector<cv::Mat>& translations,
                        std::vector<cv::Mat>& normals, bool isRANSAC)
   //------------------------------------------------------------------------------------------------------
   {
      std::vector<cv::Point3d> train_img_pts, query_img_pts;
      for (std::pair<cv::Point3d, cv::Point3d> pt : pts)
      {
         train_img_pts.emplace_back(pt.first);
         query_img_pts.emplace_back(pt.second);
      }
      homography_pose(intrinsics, train_img_pts, query_img_pts, rotations, translations, normals, isRANSAC);
   }
...
//            std::cerr << "   display_PnP " << pp.first << " Exception: " << e.what() << std::endl;
            ...
            if (is_pose)
            {
               ...
               for (size_t i=0; i<std::min(world_pts.size(), image_pts.size()); i++)
               {
                  ...
                  ppts.emplace_back(std::make_pair(world_pts[i], cv::Point2d(ipt.x, ipt.y)));
               }
               Eigen::Matrix3d R = Q.toRotationMatrix();
               project3d(K, R, t, ppts, max_error, mean_error);
