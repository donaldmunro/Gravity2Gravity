...
//-------------------------------------------------------------------------------------------------------------
   ...
   static bool random_vector3d_fixed_norm_kludge(const double norm, Eigen::Vector3d& v, std::mt19937& rand_generator)
   //-------------------------------------------------------------------------------
   {
      ...
      distribution.reset(new std::uniform_real_distribution<double>(0, norm));
      double one = vv[0] = ((*distribution)(rand_generator));
      distribution.reset(new std::uniform_real_distribution<double>(0, std::max(norm - vv.norm(), 0.0)));
      double two = vv[1] = ((*distribution)(rand_generator));
      double three = std::sqrt(std::max(norm*norm - one*one - two*two, 0.0));
      pot.clear();
...
//      assert(near_zero(v.norm() - norm, 0.00001));
      return (near_zero(v.norm() - norm, 0.00001));
   }
